...
namespace AMPMVC5.Models
{
    public class Utils
    {
        public static bool InitialiseDiary()
        {
            ...
            //LocationDBEntities1 ent = new LocationDBEntities1();
            //try
            //{
            //    for (int i = 0; i < 30; i++)
            //    {
            ...
            //        item.someKey = i;
            //        item.allDay = GetRandomValue(0, 3); // random is exclusive - we have three status enums
            //        if (i <= 5) // create a few appointments for todays date
            //        {
            //            item.start = GetRandomAppointmentTime(false, true);
            //        }
            //        else {  // rest of appointments on previous and future dates
            //            if (i % 2 == 0)
            //                item.end = GetRandomAppointmentTime(true, false); // flip/flop between date ahead of today and behind today
            //            else item.end = GetRandomAppointmentTime(false, false);
            //        }
